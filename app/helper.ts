...
export function lsTree(args: string[]) {
  if (args.length < 3)
    throw new Error("You must specify command line arguments.");
  if (args[1] != "--name-only") throw new Error("Use  --name-only");
...
export function writeTree(args: string[]) {
  let sha: string = writeTreeObj(process.cwd());
  process.stdout.write(sha);
}

function writeTreeObj(basePath: string) {
  let res: TreeType[] = [];
  const dirFiles = fs.readdirSync(basePath);
  for (let dirFile of dirFiles) {
    if (dirFile.includes(".git")) continue;
    if (fs.statSync(path.join(basePath, dirFile)).isDirectory()) {
      const dirSha = writeTreeObj(path.join(basePath, dirFile));
      let t: TreeType = { mode: "40000", sha: dirSha, name: dirFile };
      res.push(t);
    } else {
      //if it is a file
      ...
      let t: TreeType = { mode: "100644", sha: sha2, name: dirFile };
      res.push(t);
    }
  }
...
export async function copyFiles(args: string[]) {
